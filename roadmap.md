...
## 17. Zhenzhen Image G 模型参数分流修正（2026-07-26）

- `zhenzhen-image-g-v2-lowprice` 作为 `Zhenzhen_Image_G2` 节点默认模型。
- Lowprice 使用独立的 `resolution=1k|2k|4k`、顶层 `size`、顶层 `n=1..10` 和最多 16 张参考图。
- G-2 文生图与图像编辑继续固定 `1k` 并使用 `metadata.ratio`；图像编辑最多 10 张参考图。
- 前端按模型切换参数和图片槽位，后端执行同样的模型特定校验，避免隐藏字段误入请求。
- 四个既有 G2/Lowprice 示例工作流同步到六个 widgets 和 16 图输入契约。

## 18. 图片 30 路 / 视频 10 路并发改造规划（2026-08-04）

状态：已完成实现、离线回归、真实 ComfyUI 加载检查和真实接口并发验证。

对照来源：

- 原项目：`F:\AI-T8-video-onekey\ComfyUI\custom_nodes\ComfyUI_Seedance`
- 用户修改版：`G:\Comfyui-zhenzhen(1)`
- 用户版核心文件：`ComflyConcurrent.py` 与修改后的 `__init__.py`
- 原项目当前基线：完整测试 `190 passed`

### 18.1 用户修改版的实现方式

用户版没有重写原模型节点，而是在注册阶段增加了一层 submit/collect 包装：

1. 图片和视频各建立一个 `ThreadPoolExecutor`，默认工作线程分别为 30 和 10。
2. 遍历原节点注册表，根据第一个输出类型判断图片或视频节点。
3. 为符合条件的原节点动态生成“并发发射”节点，复制原节点的 `INPUT_TYPES`。
4. 发射节点为每次任务创建独立原节点实例，将原节点执行函数放入对应线程池，并立即返回 `Future`。
5. 图片和视频各提供一个“并发接收”节点；接收节点等待全部 `Future`，按连接槽位返回最多 30 张图片或 10 个视频。
6. 用户版对返回 coroutine 的旧节点使用 `asyncio.run`，对配置文件读写增加互斥锁，并通过环境变量允许调整线程数。

这种方式能让 ComfyUI 先提交多条任务、最后统一等待，核心思路有效，而且原节点仍可单独使用。

### 18.2 可直接借鉴与必须修正的部分

| 用户版机制 | 结论 | 原项目中的处理 |
| --- | --- | --- |
| submit/collect 两阶段工作流 | 采用 | 新增可选并发节点，不替换原节点 |
| 图片、视频独立线程池 | 采用 | 默认 30 / 10，互不占用工作线程 |
| 每个任务创建独立节点实例 | 采用 | 避免节点实例字段在并发任务间串线 |
| 每线程独立 HTTP Session | 原项目已具备 | 继续使用 `core/client.py` 的 `threading.local()`，增加并发回归测试 |
| 深复制原节点输入定义 | 有条件采用 | 复制后增加稳定性检查，不修改原控件选项对象 |
| 根据第一个输出自动分类全部节点 | 不直接采用 | 只包装明确登记的纯图片/纯视频节点；混合输出节点单独适配 |
| 环境变量同时决定线程数和接收插槽数 | 不采用 | 插槽永久固定为图片 30、视频 10；环境变量只降低实际工作线程，避免旧工作流插槽漂移 |
| 接收端把所有异常静默替换为占位结果 | 不作为默认 | 默认抛出原异常；用户显式选择继续模式时才返回占位结果和槽位状态 |
| 只保留原结果的第一个输出 | 不完全采用 | 并发接收输出主媒体，并保留每个槽位的非媒体结果摘要；原节点继续提供完整输出合同 |
| 导入模块时立即创建线程池 | 不采用 | 线程池延迟创建，进程结束时关闭，避免插件重载留下旧线程 |
| 全局替换配置读写函数 | 不需要 | 原项目配置读取不写共享 JSON 文件，不增加全局 monkey patch |

### 18.3 兼容性目标

- 原有 24 个注册节点的键名、输入顺序、输出顺序、模型参数、请求体和错误行为保持不变。
- 原有工作流无需迁移；不使用并发节点时，执行路径与当前版本一致。
- 并发功能通过新增节点显式启用，不在后台自动改变所有工作流的执行方式。
- 不修改模型接口契约、上传字段、查询状态、结果解析和下载格式。
- 第一阶段只覆盖图片与视频生成；音频、语音转写和 Suno 不进入这次并发改造。
- 不新增 Python 依赖，继续兼容项目当前 Python 与 ComfyUI 环境。

### 18.4 并发节点架构

建议新增 `concurrent_nodes.py`，不把线程池和动态包装逻辑继续堆入 `nodes.py`。

#### 18.4.1 固定类型与任务句柄

- 图片任务类型：`SEEDANCE_IMAGE_FUTURE`
- 视频任务类型：`SEEDANCE_VIDEO_FUTURE`
- 返回值使用轻量 `ConcurrentTaskHandle`，包含 Future、媒体类型、原节点注册键、主媒体输出索引和取消事件。
- 不把可变任务状态存入模块级字典；每个句柄只属于一次节点执行。

#### 18.4.2 线程池管理

- 图片接收节点固定 30 个输入和 30 个图片输出。
- 视频接收节点固定 10 个输入和 10 个视频输出。
- 默认工作线程为图片 30、视频 10。
- `SEEDANCE_IMAGE_CONCURRENCY` 只允许在 1 至 30 之间调整实际图片工作线程。
- `SEEDANCE_VIDEO_CONCURRENCY` 只允许在 1 至 10 之间调整实际视频工作线程。
- 环境变量变化不改变节点插槽数量、输出数量或已保存工作流结构。
- 两个线程池延迟到首次提交时创建，使用锁保证只初始化一次，并注册进程退出清理。
- 增加有界提交控制，避免一个超大工作流无限堆积未开始的 Future。

#### 18.4.3 发射节点

- 对每个明确支持的原节点生成一个并发发射节点。
- 发射节点完整复用原节点输入控件、默认值、可选输入和运行时校验。
- 每次提交创建新的原节点实例，不复用执行实例。
- 原函数返回普通值或 awaitable 时均可执行。
- 发射节点使用 `IS_CHANGED=NaN`，防止 ComfyUI 缓存已完成的 Future。
- 发射节点本身不是输出节点；只有连接并发接收节点后才进入工作流执行。

#### 18.4.4 接收节点

- 图片接收节点：`future_1` 必填，`future_2` 至 `future_30` 可选。
- 视频接收节点：`future_1` 必填，`future_2` 至 `future_10` 可选。
- 使用完成通知收集 Future，不按槽位逐个阻塞；完成顺序可以不同，但最终输出严格回到输入槽位顺序。
- 图片保持独立输出，不强行拼接不同尺寸的 Tensor，也不丢弃批次中的后续图片。
- 视频保持原 `VIDEO` 对象，不复制或重新编码已经下载的结果。
- 增加 `failure_mode`：默认 `raise`，可选 `placeholder`。
- `raise` 模式保留原节点失败语义；任一任务失败时取消尚未开始的同组任务并报告具体槽位。
- `placeholder` 模式只替换失败槽，其他任务继续；额外输出 `status_json`，记录槽位、原节点、完成状态和脱敏错误摘要。
- 未连接槽使用稳定占位对象，不影响已连接槽的序号。
...
- Seedance 文生视频、图生视频、多模态视频
- Zhenzhen Video G Omni Flash、GK v1.5、V3.1
- HappyHorse 1.1
- Wan 2.7 Spicy
- Kling Video、Kling Edit Video
- Hailuo 2.3、Hailuo H3
- Vidu Q3 Video、Vidu Q3 ShortPlay
- Zhenzhen Upscaler Video

这些节点的主输出和统一附加输出结构稳定，适合通过同一生成器创建并发发射类。

### 18.6 Midjourney 特殊处理

`Midjourney_Multi_Action` 不能按第一个输出类型自动归入图片池。它同时包含 5 个图片输出、4 个视频输出、文本和任务链结果；用户版会把 `midjourney-video` 错误归入图片池，并只取第一个图片占位输出。

规划为两个专用发射节点：

- `Midjourney 并发提交图片`：只允许最终产生图片的动作，进入图片线程池，主媒体取 `image1`。
- `Midjourney 并发提交视频`：固定视频动作，进入视频线程池，主媒体取 `video1`。

Describe 文本动作和 Inpaint 的中间阶段不混入图片/视频接收器，继续使用原节点。Midjourney 原节点的多图、多视频、按钮和完整任务链输出保持不变；并发聚合路径面向批量主媒体，状态摘要保留非媒体结果索引信息。

### 18.7 动态前端兼容

当前 Hailuo H3、Zhenzhen、Suno 和 Midjourney 前端扩展都按原注册键精确匹配。若仅动态复制 Python 类，包装节点不会获得模型切换、参数显隐和连接保护。

实施要求：

- 新增统一的并发节点名称解析助手，将包装键还原为原节点键。
- Hailuo H3 和 Zhenzhen 动态界面同时识别原节点与对应并发包装节点。
- Midjourney 专用并发节点使用受限动作列表，并继续复用原动作显隐逻辑。
- 已转换为外部输入的文本控件、已连接素材口和自定义比例控件不得因切换模型而丢失或断线。
- API Key 链接按钮只出现一次。
- 并发接收节点使用稳定尺寸，30/10 个插槽在缩放、折叠和重新加载后不重叠。
- 旧前端扩展对原节点的行为由现有测试锁定，不因名称归一化产生重复回调。

### 18.8 进度、取消与生命周期

直接让 30 个后台节点同时更新 ComfyUI 单节点进度会造成进度显示互相覆盖。规划增加仅作用于并发工作线程的运行上下文：

- 原节点串行执行继续显示当前阶段进度。
- 并发工作线程不直接发送子节点进度，接收节点按每个子任务的上传、轮询、下载阶段进度计算平均值并统一显示。
- 接收等待使用短超时完成通知，并定期检查 ComfyUI 中断状态，不做忙循环。
- 用户中断时先取消尚未开始的 Future，再通知运行中的本地轮询停止；不承诺远端已经受理的任务也会终止。
- 线程池关闭时不接受新任务，插件正常退出不遗留非守护工作线程。

### 18.9 请求与文件安全

- 继续使用 `core/client.py` 的线程局部 Session；同一工作线程复用连接，不同线程不共享 Session。
- 上传、提交、查询和下载继续使用现有重试与错误提取逻辑。
- 并发上传与提交沿用现有重试等待，并让等待支持本地取消；本次不额外引入随机延迟，避免改变原串行路径。
- 下载文件继续使用 UUID 文件名，增加并发唯一性测试。
- 配置节点输出只读使用；不把认证信息复制到 Future 描述、状态输出或日志。
- 示例工作流不保存认证信息、真实任务标识、临时结果地址或运行结果。

### 18.10 自动化测试计划

新增 `tests/test_concurrent_nodes.py`，并扩展前端、注册和工作流测试。

#### 注册与兼容

- 原 `NODE_CLASS_MAPPINGS` 和显示名完整保留。
- 新并发键无冲突，原类对象和原 INPUT/RETURN 合同未被修改。
- 所有现有 190 项测试继续通过。
- 原工作流在不添加并发节点时，模拟请求体和返回结构与改造前一致。

#### 并发上限与隔离

- 使用 Barrier 和活动计数器证明图片最多同时运行 30 条、视频最多同时运行 10 条。
- 图片池和视频池同时工作，任一池满载不占用另一池工作线程。
- 环境变量降低工作线程后，节点仍保持固定 30/10 插槽。
- 超过单个接收节点容量的提交受到有界控制，不无限增长。
- 每个执行使用不同节点实例；任务参数、结果、异常和下载路径不会串线。
- 每个线程拥有独立 HTTP Session，并在线程内复用。

#### Future 与结果

- 普通返回、`{"ui": ..., "result": (...)}` 返回和 awaitable 返回均正确解包。
- Future 乱序完成时，结果仍按输入槽位输出。
- 不同尺寸图片不会被错误拼接或丢弃。
- `raise` 模式传播原异常，`placeholder` 模式只替换失败槽。
- 一条失败不污染其他已完成任务；接收器状态摘要不包含认证信息。
- Midjourney 图片和视频分别取正确输出索引，文本与中间动作不会误入媒体池。

#### 取消与前端

- 排队 Future 可取消，运行中任务能观察本地取消事件并停止继续查询或下载。
- 聚合等待无忙循环，并能持续更新完成数量。
- 所有动态模型界面在包装节点上与原节点一致。
- 外部 STRING 连接、自定义比例、模型切换、保存和重载均保持连接与取值。
- API Key 按钮不重复，节点控件无重叠。

### 18.11 示例工作流

至少新增两份不依赖第三方节点的示例：

- `并发图片30路示例.json`：多个图片发射节点连接图片接收节点，展示槽位顺序和单槽失败处理。
- `并发视频10路示例.json`：多个视频发射节点连接视频接收节点，展示独立参数和结果顺序。
...
大工作流用于结构和压力验证，小工作流用于用户快速确认安装是否正常。所有工作流保留空配置，不能保存运行结果。

### 18.12 分阶段实施

#### 阶段 A：并发运行时骨架

- 新增任务句柄、延迟线程池、有界提交、取消上下文和结果解包器。
- 使用轻量假节点完成 30/10 并发上限、乱序完成、异常和取消测试。

#### 阶段 B：纯图片与纯视频节点包装

- 按明确白名单生成发射节点。
- 合并注册表时使用新字典，不修改 `nodes.py` 暴露的原始映射对象。
- 完成图片/视频接收节点和状态摘要。

#### 阶段 C：动态界面适配

- 增加包装键归一化。
- 逐一验证 Hailuo H3、Zhenzhen Image G/NB/V3.1 等动态界面。
- 在真实 ComfyUI 中检查创建、连线、切换、保存、重载和节点尺寸。

#### 阶段 D：Midjourney 专用包装

- 图片动作和视频动作分开进入对应线程池。
- 锁定主媒体索引和不支持动作清单。
- 保留原 Midjourney 节点作为完整多结果与任务链路径。

#### 阶段 E：离线压力与回归

- 本地假服务同时运行 30 条图片任务和 10 条视频任务。
- 注入随机延迟、短暂网络错误、单槽失败、取消和不同尺寸图片。
- 运行完整测试、Python 编译、前端 JavaScript 检查、工作流 JSON 检查和敏感信息扫描。

#### 阶段 F：真实接口分级验证

- 使用临时环境变量提供测试认证信息，不写入文件。
- 先验证图片 2 路、视频 2 路并发，再逐级提高并发数。
- 最终分别验证图片 30 路和视频 10 路确实同时在途、全部正确归槽并可读取。
- 图片与视频同时运行一次，确认两个线程池互不阻塞。
- 真实记录只保留日期、并发数量、成功/失败数量、是否正确下载和是否串线，不保存认证信息或运行标识。

#### 阶段 G：文档与发布

- 更新 README 的并发节点使用方式和环境变量。
- 更新版本号及被忽略的 `skill.md` 非敏感验证结论。
- 未经用户明确要求不提交或推送 GitHub。

### 18.13 预计改动文件

- `concurrent_nodes.py`：任务句柄、线程池、动态发射类、接收节点
- `core/runtime.py`：并发线程上下文、进度抑制和取消检查
- `core/client.py`：仅增加并发上下文感知的可取消等待，默认路径保持原行为
- `__init__.py`：合并原节点与并发节点映射
- `web/js/concurrent_node_ui.js`：包装键识别和接收节点前端行为
- `web/js/hailuo_h3_model_ui.js`
- `web/js/zhenzhen_model_ui.js`
- `web/js/midjourney_action_ui.js`
- `tests/test_concurrent_nodes.py`
- `tests/test_frontend_extension.py`
- `examples/` 下四份并发示例
- `README.md`
- `pyproject.toml`
- 被忽略的 `skill.md`，仅在真实验证后记录非敏感结论

### 18.14 完成标准

- 不使用并发节点时，原节点、原工作流和原 190 项测试行为不变。
- 图片接收节点固定 30 路，视频接收节点固定 10 路。
- 离线压力测试证明 30/10 的实际重叠执行、独立限流、结果归槽和异常隔离。
- 真实 ComfyUI 中的模型动态界面、外部文本连接和素材连接在包装节点上正常。
- Midjourney 视频不会被误分类为图片任务。
- 默认失败行为不被静默吞掉；继续模式能保留其他成功槽。
- 中断工作流后不会继续无限等待本地 Future。
- 图片、视频下载文件名无冲突，结果可被对应 ComfyUI 类型读取。
- 四份示例工作流可加载且不包含认证信息或真实运行数据。
- 完整测试、编译、前端检查、JSON 检查和安全扫描全部通过。

### 18.15 实施结果（2026-08-04）

- 新增 `concurrent_nodes.py`：固定 30 路图片、10 路视频插槽，图片/视频独立延迟线程池、有界提交、任务句柄、取消和两种失败模式。
- 新增 `core/runtime.py`，并让客户端轮询与重试等待支持并发任务取消；原串行节点继续使用原有进度与等待行为。
- 明确包装 4 个纯图片节点和 15 个纯视频节点；Midjourney 图片与视频使用两个专用提交节点，不按首个输出自动误分类。
- 原始注册表保持 24 个节点不变，插件合并注册后共 47 个节点；原节点类、输入合同和输出合同未被替换。
- Hailuo H3、Zhenzhen 与 Midjourney 动态前端已识别并发包装节点，Midjourney 两种包装只显示各自允许的动作。
- 新增四份并发示例：图片/视频 2 路最小验证、图片 30 路完整示例、视频 10 路完整示例。
- 离线压力测试使用活动计数器确认 30 个图片工作线程与 10 个视频工作线程能够同时重叠执行，结果乱序完成后仍按槽位归位。
- 真实接口以同一批次提交 30 个图片任务和 10 个 4 秒视频任务；图片 30/30、视频 10/10 完成，失败数均为 0。
- 30 个图片结果均转换为有效四维 ComfyUI `IMAGE` 张量；10 个视频均通过 MP4 字节检查和 `ffprobe`，为 H.264、864x496、4.042 秒。
- 真实验证只通过进程环境注入认证信息，临时视频按精确文件名验证后清理，仓库未保存认证信息、任务标识、结果地址或运行响应。
- 修复动态控件缩放重叠的共同根因：新增共享 `dynamic_widget_ui.js`，使用 ComfyUI 可识别的 converted-widget 隐藏类型，统一替换 Zhenzhen、Hailuo H3、Suno 与 Midjourney 的旧隐藏逻辑。
- 真实 ComfyUI 画布已验证原始 Zhenzhen Image G2、Hailuo H3、Suno、Midjourney 及并发 G2 包装节点；切换到会隐藏参数的模型或动作后手动调整节点尺寸，隐藏行均未重绘或压住相邻控件。
- 用户两路 Lowprice 图像编辑工作流暴露了上游提示词 5 到 5000 字符限制；已将模型专属校验前移并覆盖并发包装节点，避免 4 字符提示词在素材上传后才由接收节点报告 HTTP 400。
- 修复后使用同一张本地参考图真实运行两路 Lowprice 图像编辑并发任务，两路均成功生成并通过共享接收节点返回 `[1, 1254, 1254, 3]` 图片张量，失败槽为 0。
- 兼容 ComfyUI 0.30 的校验错误映射：并发包装节点不再直接暴露原节点多参数自定义校验，模型联动校验改在 `submit()` 创建 Future 前同步执行，避免一条提示词错误被复制成节点全部输入无效。
- 用户两路工作流确认 API 已完成后接收节点仍等待；分阶段真实复现定位到其中一路结果下载首轮连接持续约 143 秒后以 `ChunkedEncodingError` 结束，第二次下载随即成功，排除 Future 死锁和串行提交。
- 图片结果下载改为流式读取，连接与单次读取均使用短超时，并设置单次 45 秒总时限后重试；并发接收节点同时汇总子任务原有的 0 到 100 阶段进度，因此生成完成后的下载等待可见且有界。
- 改造后再次使用同一参考图运行两路 Lowprice 图像编辑完整链路：两路上传和提交重叠执行，最终 2/2 成功，均返回 `[1, 1254, 1254, 3]` 图片张量；本轮两个结果下载分别约 4.6 秒。
- 最终完整回归为 210 项测试与 1027 个子测试通过，另完成 Python 编译、前端脚本、示例工作流、敏感信息及新增文本策略检查。

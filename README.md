...
- 新增独立的 `MiniMax-H3 V2 多模态视频生成` 节点，严格使用模型 `MiniMax-H3` 和 V2 提交、查询及结果协议，不改变已有 MiniMax H3 OW 节点和工作流。
- 支持文生视频、首尾帧、多模态参考和驱动音频模式；可接入最多 9 张参考图、3 段参考视频、3 段参考音频，并提供 10 路视频并发提交版本。
- 新节点支持 480P/768P、标准随机种子缓存、`skip_error`、动态素材控件和可靠媒体下载，新增四份不含凭据的示例工作流。
- 完成 4 秒、480P 文生视频真实上传、提交、轮询、下载和 MP4 校验；完整离线回归通过 402 项测试。
...
- 严格区分模型参数：T2V 使用固定比例，I2V 支持首帧与可选尾帧且不发送比例；两者支持 5 到 15 秒及 480P/768P。
- 新节点支持标准随机种子缓存、`skip_error`、动态素材控件和可选 10 路视频并发提交，并新增两份无凭据示例工作流。
- 两个模型均完成 5 秒、480P 的真实上传、提交、轮询、下载与 MP4 校验；完整离线回归通过 390 项测试。
...
- 新增 `Wan 3.0 图生/参考生视频（4 合 1）` 节点，覆盖国内/海外 I2V 与 R2V；I2V 支持首尾帧，R2V 支持最多 10 图、5 视频、5 音频及可选文件或网页参考。
- 节点按模型动态显示有效素材与参数，支持原生随机种子、`skip_error` 和视频 10 路并发提交；无效参数会在上传素材前拦截。
- 新增 4 份逐模型安全示例工作流。四个模型均以 2 秒、480P 完成真实上传、提交、轮询、下载和 H.264 MP4 验证，两条 R2V 路径均同时使用了本地图片、视频和音频。
- 完整离线回归通过 373 项测试，177 份示例工作流通过 JSON 与敏感信息审计，15 个前端脚本通过语法检查。

## v0.7.1（2026-08-22）

- 根据真实接口行为，将混元 3D v3.1 图生 3D 的提示词改为必填，并保留 1 到 8 张有序多视图输入。
- 视频提交日志会递归隐藏素材 URL 与运行时标识，请求体本身保持不变。
- Lowprice 参考视频与混元图生 3D 已完成真实节点全链路验证；GLB 结果通过完整性检查及 ComfyUI 原生预览、保存验证。
...
- 保留旧内部节点类型 `FashVSR_Video_Upscale` 作为兼容入口，已有工作流无需重建。

## v0.5.24（2026-08-18）

- 新增独立 `FlashVSR 480P 视频超分` 节点，严格使用模型名 `FlashVSR_video_upscale` 和兼容端点 `/v1/video/generations`，请求仅发送一个 `metadata.video_url`。
...
- 新增可直接加载的 FlashVSR 示例工作流，API Key 与运行结果保持空白；真实节点测试以 854×480、3 秒 H.264 输入完成上传、提交、轮询和下载，结果为有效 1920×1024 H.264 MP4。
- 完整离线回归通过 335 项测试，155 份示例工作流通过 JSON 与敏感信息审计。

## v0.5.23（2026-08-16）

- 修复任务创建响应丢失时自动重放 POST、可能重复创建同一生成任务的问题。视频、图片、音频、语音转写、Suno、Midjourney 与 Context IR 统一只在确认请求尚未发送的连接失败或明确 `429` 时重试；读取超时、连接中断与 `5xx` 不再自动重放。
- 图片、视频、音频和通用结果下载新增独立安全上限，Python、无代理直连与系统 `curl` 三条下载路径都会同时检查响应长度和实际接收字节；超限立即清理 `.part` 文件且不再重复下载。
- 默认下载上限分别为图片 64 MiB、音频 512 MiB、通用文件 1024 MiB、视频 8192 MiB，可在启动 ComfyUI 前通过 `SEEDANCE_IMAGE_MAX_MIB`、`SEEDANCE_AUDIO_MAX_MIB`、`SEEDANCE_FILE_MAX_MIB`、`SEEDANCE_VIDEO_MAX_MIB` 调整。
- `SEEDANCE_SSL_VERIFY=0` 暂时保留兼容，但现在会发出明确的安全弃用警告且不再静默关闭底层 TLS 警告；`SEEDANCE_CA_BUNDLE` 优先于旧开关，推荐使用自定义 CA 或 Windows 证书库。
- 节点输入输出、并发 Future、`skip_error`、seed 缓存和现有工作流结构保持不变；新增网络可靠性专项回归覆盖全部七类任务提交与三种下载通道，完整离线回归通过 325 项测试。

## v0.5.22（2026-08-15）

- 修复部分 ComfyUI 版本中并发提交节点把一次模型参数校验错误重复展开到 `api_config`、全部素材插槽和所有控件，导致整个工作流显示数十项“无效输入”的问题。
- 全部图片与视频并发提交节点改为独立包装类，不再继承原生成节点的队列验证函数；节点名称、输入端口、控件顺序、Future 输出和旧工作流连线保持不变。
- 原生成节点的严格模型校验仍会在 Future 创建前同步执行，参数错误不会进入并发池；开启 `skip_error` 时仍只隔离当前一路，不影响其余任务。
- 使用本机 ComfyUI 队列验证实测 3 个并发节点共用图片和 API 配置时为 0 项错误，无效模型只定位到 `model` 自身；完整离线回归通过 313 项测试和 1499 个子测试。
...
- `Seedream / Dola Seedream v5 Pro 图层拆分（2 合 1）` 节点新增海外模型 `dola-seedream-5.0-pro-layer-decomposition`，原国内模型仍为默认值。
- 两个模型共用文档规定的单图输入、可选提示词、`auto` / `1k` / `1.5k` / `2k` 分辨率和 PNG/JPEG 输出参数。
- 节点继续按 API 返回顺序输出完整 IMAGE/MASK 列表，不合批、不缩放、不去重；模型选择追加在原有 seed 控制之后，旧工作流不会发生控件值错位，未保存模型字段时仍按国内模型执行。
- 新增 Dola 海外图层拆分工作流，并更新国内工作流显式保存模型选择；API Key 与运行结果保持空白。
- 海外 Dola 模型已使用 512×512 本地图片、`auto` 和 PNG 完成真实节点验证，返回 1 张底图和 4 个异尺寸图层，5 组 IMAGE/MASK 均成功下载并解码。
- 完整离线回归通过 300 项测试，139 份示例工作流通过 JSON 与敏感信息审计，10 个前端脚本通过语法检查。

## v0.5.18（2026-08-11）

- 新增 `MiniMax H3 Context IR 提示词增强（3 合 1）` 节点，包含 `minmax-h3-context-ir-text`、`minmax-h3-context-ir-image` 和 `minmax-h3-context-ir-multimodal`。
- 节点严格使用兼容端点 `POST /v1/video/generations` 与 `GET /v1/video/generations/{id}`，从成功响应的 `result_text` 输出可继续连接到视频生成节点的增强提示词。
- Text 支持 4 到 15 秒和 6 种固定比例；Image 支持 1 到 2 张首尾帧；Multimodal 支持最多 9 图、3 视频和 3 音频，并支持可选或自适应比例。
- 多模态素材真实验证确认：图片使用顶层 `images`，视频必须使用 `metadata.video_urls` 复数数组，音频使用 `metadata.audio_url` 数组；图、视频和音频可在同一任务中一起提交。
- 前端按模型动态显示适用比例和素材插槽；节点支持 `skip_error` 与标准 ComfyUI seed 缓存控制，但不加入图片或视频 Future 并发包装器。
- 新增 Text、Image、Multimodal 三份示例工作流，全部保持 API Key、任务号和运行结果为空。
- 三个模型均使用 4 秒参数完成真实节点测试并返回非空增强提示词；Image 使用正常尺寸参考图，Multimodal 同时上传图片、H.264 MP4 和 WAV。
- 完整离线回归通过 299 项测试，138 份示例工作流通过 JSON 与敏感信息审计；整合包成功导入 59 个节点。
...
- 支持独立的 MiniMax-H3 V2 多模态视频节点：文生、首尾帧、9 图、3 视频、3 参考音频及单路驱动音频
...
| `MiniMax-H3 多模态视频生成` | MiniMax V2 文生、首尾帧、多模态参考及音频驱动视频 | 必填 `prompt`、4 到 15 秒（驱动音频可到 60 秒）、480P/768P、固定/自适应比例、9 图/3 视频/3 参考音频/1 驱动音频 |
...
独立的 Hailuo H3 Max 节点同样使用 `/v1/videos`：
...
独立的 MiniMax-H3 节点使用 MiniMax V2 协议：提交 `POST /v2/video_generation`，轮询 `GET /v2/query/video_generation/{task_id}`，成功后从 `task.content.url` 下载视频。模型名固定为区分大小写的 `MiniMax-H3`。

节点要求非空提示词并支持 480P/768P。普通生成支持 4 到 15 秒，连接独立 `drive_audio` 后支持 4 到 60 秒；可组合首帧、尾帧、最多 9 张参考图、3 个参考视频（每路可设起始秒）、3 条参考音频及 1 条驱动音频。纯文生必须使用固定比例；`adaptive`/`auto` 仅在关键帧请求中可用；`api_default` 用于让参考或关键帧请求省略比例。音频控制严格区分 `lock_source`、`remix_source`、`reference_only` 与 `native`。节点同时提供 `skip_error`、仅用于 ComfyUI 缓存的标准 seed，以及 10 路视频并发提交版本。
...
| 模型 | 用途 | 素材与比例 |
| --- | --- | --- |
| `minmax-h3-context-ir-text` | 纯文本增强 | `prompt` 必填；必须选择 6 种固定比例之一 |
| `minmax-h3-context-ir-image` | 首尾帧上下文增强 | `image1` 必填，`image2` 可选；不发送比例 |
| `minmax-h3-context-ir-multimodal` | 多模态上下文增强 | 最多 9 图、3 视频、3 音频；比例可省略或设为 `adaptive` |

三个模型的 `prompt` 均为 1 到 7000 字符，`seconds` 为字符串 `4` 到 `15`。多模态图片发送到顶层 `images`，视频发送到 `metadata.video_urls`，音频发送到 `metadata.audio_url`。前端会隐藏当前模型不使用的素材和比例输入。
...
| `audio1` ... `audio5` | 仅 R2V 使用的参考音频 |
| `file_url` / `link_url` | 仅 R2V 使用的可选文件或网页参考，二选一 |
| `seed` | 0 到 2147483647 的原生随机种子，支持 ComfyUI 生成后控制 |
...
MiniMax H3 Context IR 节点参数：

| 参数 | 说明 |
| --- | --- |
| `model` | Text、Image 或 Multimodal 三个提示词增强模型 |
| `prompt` | 待增强的视频提示词，1 到 7000 字符 |
| `seconds` | 目标视频时长，`4` 到 `15` 秒 |
| `ratio` | Text 必须使用固定比例；Multimodal 可使用固定比例、`adaptive` 或 `api_default`；Image 不提交 |
| `image1` / `image2` | Image 的必填首帧和可选尾帧；Multimodal 的前两张参考图 |
| `image3` ... `image9` | 仅 Multimodal 使用的可选参考图 |
| `video1` ... `video3` | 仅 Multimodal 使用的可选参考视频 |
| `audio1` ... `audio3` | 仅 Multimodal 使用的可选参考音频 |
| `api_config` | 可选，复用 `Seedance API Config` 的地址与 API key |
| `skip_error` | 开启后失败时返回空提示词和错误响应，不中断工作流 |
| `seed` | 仅用于 ComfyUI 缓存控制，不发送给 Context IR API |

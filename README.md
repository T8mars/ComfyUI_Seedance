...
- 视频结果使用 8 秒连接、45 秒读取和单次 180 秒总时限；首次网络或完整性错误会立即切换系统下载通道，失败后再通过全新 Python Session 短间隔重试。
...
- 音频任务使用 `/v1/audio/generations` 独立状态规则轮询；结果下载使用原子落盘与双通道恢复，成功解码后才返回 ComfyUI `AUDIO`。

...
class ConcurrentNodeTests(unittest.TestCase):
    ...
    def test_package_keeps_base_mapping_unchanged_and_adds_concurrent_nodes(self):
        self.assertEqual(len(nodes.NODE_CLASS_MAPPINGS), 51)
        self.assertEqual(len(concurrent_nodes.CONCURRENT_NODE_CLASS_MAPPINGS), 42)
        self.assertEqual(len(plugin.NODE_CLASS_MAPPINGS), 93)
